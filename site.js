(function(){
  ...
  function init(){
    ...
    if(!input||!box) return;
    input.addEventListener('input', function(){ renderResults(input.value, box); });
    input.addEventListener('focus',  function(){ if(input.value.trim()) renderResults(input.value, box); });
